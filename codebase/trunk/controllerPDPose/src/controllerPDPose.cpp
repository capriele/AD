...
bool controllerPDPose_t::doComputations()
{
    controllerPDPose_t* podWorker = this;
    ...
    double deltax = cos(yaw_hat) * (poseRef.position[0] - stateVariances.position[0]) + sin(yaw_hat) * (poseRef.position[1] - stateVariances.position[1]);
    double deltay = -sin(yaw_hat) * (poseRef.position[0] - stateVariances.position[0]) + cos(yaw_hat) * (poseRef.position[1] - stateVariances.position[1]);
    double deltaz = poseRef.position[2] - stateVariances.position[2];


    double pitch_ref = -powerAdjust.pXYAdjustPDPOS * Pxy * deltax + powerAdjust.dXYAdjustPDPOS * Dxy * stateVariances.veloPositionBody[0];
    double roll_ref = powerAdjust.pXYAdjustPDPOS * Pxy * deltay - powerAdjust.dXYAdjustPDPOS * Dxy * stateVariances.veloPositionBody[1];


    //PD control state error to torque-equivalent about bodyframe axis

    double tau_p 		= powerAdjust.ppAdjustPDO * Pp * (pitch_ref - pitch_hat) - powerAdjust.dpAdjustPDO * Dp * stateVariances.veloOrientBody[1] + powerAdjust.pBiasPDO; //@TODO check convention, see PODdescriptions!
    double tau_r 		= powerAdjust.prAdjustPDO * Pr * (roll_ref - roll_hat)  - powerAdjust.drAdjustPDO * Dr * stateVariances.veloOrientBody[0] + powerAdjust.rBiasPDO;
    ...
    double totalThrust =    powerAdjust.pZAdjustPDPOS * Pz * deltaz - powerAdjust.dZAdjustPDPOS * Dz * stateVariances.veloPositionBody[2] - QUADMASS * GRAVITY - powerAdjust.tBiasPDO; // "-" because thrust "upwards" is equals negative thrust!
    ...
    motorsWsRef.wsRef[0] = sqrt(-THRUST2OMEGA2 * (ATOTALTHRUST * totalThrust + ATAUYAW * tau_y - ATAUPR * tau_p - ATAUPR * tau_r));
    motorsWsRef.wsRef[1] = sqrt(-THRUST2OMEGA2 * (ATOTALTHRUST * totalThrust - ATAUYAW * tau_y - ATAUPR * tau_p + ATAUPR * tau_r));
    motorsWsRef.wsRef[2] = sqrt(-THRUST2OMEGA2 * (ATOTALTHRUST * totalThrust + ATAUYAW * tau_y + ATAUPR * tau_p + ATAUPR * tau_r));
    motorsWsRef.wsRef[3] = sqrt(-THRUST2OMEGA2 * (ATOTALTHRUST * totalThrust - ATAUYAW * tau_y + ATAUPR * tau_p - ATAUPR * tau_r));
    ...
    // statusDrone - publish
    lcm.publish("motorsWsRefPDPose", &motorsWsRef); 	//choose channelName here!
...
bool controllerPDPose_t::updateStatus()
{
    controllerPDPose_t* podWorker = this;
