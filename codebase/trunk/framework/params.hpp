...
#define MAXPODDELAY_X 1.5 	 // POD computation interval can take at max MAXPODDELAY_X-times the POD's callinterval before error is issued //@TODO this should be lower!
#define MAXAGEMSGS_X 2.5  	 // stored message from a subscribed channel is considered out of date if most recent message was stored earlier than MAXAGEMSGS_X-times the expected receive interval of that message //@TODO this should be lower! currently, imuAcuiqistion misses some USB readings -> on update over once cycle -> 2x longer publishimuRawdata-delta
...
//SENSOR STUFF
#define CALIBINTERVAL_IMU 3000	// length of calibration interval for IMU [milliseconds]
